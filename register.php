...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!isset($_POST["email"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      ...
      $checkEmailExists = $link->prepare($sql);
      $checkEmailExists->bind_param("s", $email);
      if ($checkEmailExists->execute()) {
        $checkEmailExistsResult = $checkEmailExists->get_result()->fetch_assoc();
        if (!empty($checkEmailExistsResult)) {
  ...
  if (empty($username_err) && empty($email_err) && empty($password_err)) {
    ...
    $registerUser = $link->prepare($sql);
    $registerUser->bind_param("sss", $username, $email, $param_password);
